      ...
      <div class="w-full flex flex-row justify-around items-stretch">
        ...
        <div class="flex flex-col flex-shrink w-225px px-4">
          <form-input
            ...
            required
          />
          <base-button href="/" class="text-only text-sm h-0 mt-px"
            >Forgot password?</base-button
          >
        </div>
        <div class="flex items-center self-center max-h-39px pr-4">
          <base-button subType="button" @click="login" class="btn secondary"
            >Log in</base-button
          >
        </div>
      </div>
...
export default {
  ...
  computed: {
    isDisabled() {
      ...
      const result =
        this.username.length > 0 && this.password.length > 0 ? false : true;
      return result;
    },
  },
};
